...
@Component({
  ...
  providers: [ConfirmationService, MessageService, ProductService, CartService, RaffleService]
})
export class BasketComponent implements OnInit {
  ...
  chothen(giftId: number) {
    if (this.RaffleService.isRaffling == true) {
      Swal.fire({
        title: 'אינך יכול להוסיך לסל ההגרלה כבר בוצעה',
        text: `😌`,
        icon: 'error',
    ...
    Swal.fire({
      title: 'A new gift added to your cart',
      text: `😀`,
      icon: 'success',
      confirmButtonText: 'OK',
    })
